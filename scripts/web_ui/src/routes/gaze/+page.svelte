<script lang="ts">
	...
	import GazeBarChart from './GazeBarChart.svelte';
	import GazeLivePercentage from './GazeLivePercentage.svelte';
	...
	let detections = $derived.by(() => {
		let all_detections: any[] = metadata.metadata;
		if (!all_detections) {
			return [];
		}
		...
		new_detections = new_detections.concat(all_detections);
		return new_detections;
	});
	let last_detections = $derived.by(() => {
		if (!metadata.processed_data) {
			return [];
		}
		if (!metadata.processed_data.last_detections) {
			return [];
		}
		// return metadata.processed_data.last_detections.map((last_detection: any) => {
		// 	return { ...last_detection, face_img: '' };
		// });
		return metadata.processed_data.last_detections
			.map(
				(last_detection: {
					track_id: number;
					enter_time: string;
	...
	onMount(() => {
		...
		socket.on('recv_image_meta', (data) => {
			...
			data.metadata.metadata.sort((a: any, b: any) => a.track_id - b.track_id);
			metadata = data.metadata;
		});
	});
	// $inspect('pinned_track_ids', pinned_track_ids);
	// $inspect('last_detections', last_detections);
</script>

<DemoPage demo={demos['gaze']}>
	{#if loading}
	...
	{:else}
		<div class="grid h-full w-full gap-2 px-2 [grid-template-columns:repeat(14,minmax(0,1fr))]">
			...
			<div class="col-span-11 grid h-full min-h-[300px] w-full grid-rows-5 gap-2">
						...
						<Table.Root>
							<Table.Header>
								<Table.Row>
									...
									<Table.Head>Longest Gaze</Table.Head>
									<Table.Head>Gender</Table.Head>
									<Table.Head>Age</Table.Head>
									<!-- <Table.Head>Face Image</Table.Head> -->
								</Table.Row>
							</Table.Header>
							<Table.Body>
								{#each last_detections as last_detection}
									<Table.Row transitionSlideRight={true}>
										<Table.Head>{last_detection.track_id}</Table.Head>
										<Table.Head>{last_detection.enter_time}</Table.Head>
										<Table.Head>{last_detection.exit_time}</Table.Head>
			...
			<!-- <div class="col-span-1 rounded-md bg-gradient-to-b from-slate-600 to-slate-800"></div> -->
				...
				<ScrollArea class="h-full w-full">
					{#each detections as detection}
						<Card.Root
							class={cn(
								'w-full',
								// detection.is_frontal ? ['bg-headerBackground', 'text-headerText'] : ['bg-cardBackground', 'text-cardText'],
