<script lang="ts">
	...
	import MissingFace from '$lib/assets/missing_face.jpg';
	...
	$inspect('historical', historical_people_tracks);
	// $inspect('metadata', metadata);
	// $inspect('frame', frame);
	// $inspect('fps_count', fps_count);
</script>

<DemoPage demo={demos['gazev3']}>
	{#if loading}
						...
						<Table.Root>
							<Table.Header>
								<Table.Row>
									...
									<Table.Head>FaceReID</Table.Head>
							...
							<Table.Body>
								{#each historical_people_tracks as last_detection}
									<Table.Row transitionSlideRight={true}>
										...
										<Table.Head>{last_detection.face_reid}</Table.Head>
										<Table.Head>
											<img
						...
						>
							<div class="grid grid-cols-5 gap-1 p-2">
								<div class="col-span-2 max-h-[120px]">
									<img
										src={detection.face_img === ''
											? MissingFace
											: `data:image/jpeg;base64,${detection.face_img}`}
										alt=""
										class="h-full w-full rounded-md object-fill"
									/>
								</div>
									...
									<Table.Root class="table-fixed">
										<Table.Body>
											...
											<Table.Row>
												...
												<Table.Cell pwidth={1}
													>{detection.gender === '' ? 'N.A.' : detection.gender}</Table.Cell
												>
											...
											<Table.Row>
												...
												<Table.Cell pwidth={1}
													>{detection.age === '' ? 'N.A.' : detection.age}</Table.Cell
												>
											</Table.Row>
											<Table.Row>
											...
											<Table.Row>
												...
												<!-- <Table.Cell pwidth={1}>{detection.face_reid}</Table.Cell> -->
												<Table.Cell pwidth={1}
													>{detection.face_reid === '' ? 'N.A.' : detection.face_reid}</Table.Cell
												>
											</Table.Row>
										</Table.Body>
									</Table.Root>
