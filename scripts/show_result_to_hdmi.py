...
def inference_function(np_outputs, img, frame_time: float, cam_metadata):
    demo_pipeline.show_result(np_outputs, img, frame_time, cam_metadata)
...
if __name__ == "__main__":
    ...
    config.show_result.cv2_show_window = config.show_result.cv2_show_window if args.cv2_show_window is None else args.cv2_show_window
