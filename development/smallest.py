...
def thread_process(jobs: queue.Queue):
    while (job := jobs.get()) is not None:
        np_outputs, img = job[0], job[1]
        logger.info("Got an inference")
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if len(camera_frame_times) > 2 and i == 0:
            logger.info(
        ...
        if metadata is not None:
            ...
            if np_outputs is not None:
                ...
                if len(inference_frame_times) > 2 and j == 0:
                    logger.info(
