...
from sdk.cam_controls import read_camera_controls, save_camera_controls
...
class CameraControls():
    def __init__(self):
        ...
        camera_controls = read_camera_controls()
        self.load_dict(camera_controls)
    ...
    def load_dict(self, camera_controls: dict):
        """
        Loads a given camera_controls dict into the object
        """
        if "AeConstraintMode" in camera_controls:
            self.include_aeConstraintMode = True
            self._aeConstraintMode = camera_controls["AeConstraintMode"]
        if "AeEnable" in camera_controls:
            self.include_aeEnable = True
            self._aeEnable  = camera_controls["AeEnable"]
        if "AeExposureMode" in camera_controls:
            self.include_aeExposureMode = True
            self._aeExposureMode = camera_controls["AeExposureMode"]
        if "AeFlickerMode" in camera_controls:
            self.include_aeFlickerMode = True
            self._aeFlickerMode = camera_controls["AeFlickerMode"]
        if "AeFlickerPeriod" in camera_controls:
            self.include_aeFlickerPeriod = True
            self._aeFlickerPeriod = camera_controls["AeFlickerPeriod"]
        if "AeMeteringMode" in camera_controls:
            self.include_aeMeteringMode = True
            self._aeMeteringMode = camera_controls["AeMeteringMode"]
        if "AfMode" in camera_controls:
            self.include_afMode = True
            self._afMode = camera_controls["AfMode"]
        if "LensPosition" in camera_controls:
            self.include_lensPosition = True
            self._lensPosition = camera_controls["LensPosition"]
        if "AfMetering" in camera_controls:
            self.include_afMetering = True
            self._afMetering = camera_controls["AfMetering"]
        if "AfPause" in camera_controls:
            self.include_afPause = True
            self._afPause = camera_controls["AfPause"]
        if "AfRange" in camera_controls:
            self.include_afRange = True
            self._afRange = camera_controls["AfRange"]
        if "AfSpeed" in camera_controls:
            self.include_afSpeed = True
            self._afSpeed = camera_controls["AfSpeed"]
        if "AfTrigger" in camera_controls:
            self.include_afTrigger = True
            self._afTrigger = camera_controls["AfTrigger"]
        if "AfWindows" in camera_controls:
            self.include_afWindows = True
            self._afWindows = camera_controls["AfWindows"]
        if "AnalogueGain" in camera_controls:
            self.include_analogueGain = True
            self._analogueGain = camera_controls["AnalogueGain"]
        if "AwbEnable" in camera_controls:
            self.include_awbEnable = True
            self._awbEnable = camera_controls["AwbEnable"]
        if "AwbMode" in camera_controls:
            self.include_awbMode = True
            self._awbMode = camera_controls["AwbMode"]
        if "Brightness" in camera_controls:
            self.include_brightness = True
            self._brightness = camera_controls["Brightness"]
        if "ColourGains" in camera_controls:
            self.include_colourGains = True
            self._colourGains = camera_controls["ColourGains"]
        if "Contrast" in camera_controls:
            self.include_contrast = True
            self._contrast = camera_controls["Contrast"]
        if "ExposureTime" in camera_controls:
            self.include_exposureTime = True
            self._exposureTime = camera_controls["ExposureTime"]
        if "ExposureValue" in camera_controls:
            self.include_exposureValue = True
            self._exposureValue = camera_controls["ExposureValue"]
        if "FrameDurationLimits" in camera_controls:
            self.include_frameDurationLimits = True
            self._frameDurationLimits = camera_controls["FrameDurationLimits"]
        if "HdrMode" in camera_controls:
            self.include_hdrMode = True
            self._hdrMode = camera_controls["HdrMode"]
        if "NoiseReductionMode" in camera_controls:
            self.include_noiseReductionMode = True
            self._noiseReductionMode = camera_controls["NoiseReductionMode"]
        if "Saturation" in camera_controls:
            self.include_saturation = True
            self._saturation = camera_controls["Saturation"]
        if "ScalerCrop" in camera_controls:
            self.include_scalerCrop = True
            self._scalerCrop = camera_controls["ScalerCrop"]
        if "Sharpness" in camera_controls:
            self.include_sharpness = True
            self._sharpness = camera_controls["Sharpness"]

    def changed_dict(self) -> dict:
        d = self.dict()
        nd = {}
        for k, v in d.items():
            if v[0]:
    ...
    def changed_yaml(self) -> str:
        d = self.changed_dict()
        return yaml.dump(d, default_flow_style=False)
    
    def save_to_project_camera_control(self):
        camera_controls = self.changed_dict()
        save_camera_controls(camera_controls)
